...
class Robot:
    def __init__(self, i, x, y, heading, radius, max_linear_speed, max_angular_speed, FOV, n_beams, laser_range, clock, detection_threshold, saturation_threshold, odor_alpha, plume_lost_time):
        ...
        self.cmd_sub = rospy.Subscriber('/' + self.name + '/cmd_vel', String, self.cmd_callback)
        self.odom_pub = rospy.Publisher('/' + self.name + '/odom', Float32MultiArray, queue_size=1)
        self.gas_pub = rospy.Publisher('/' + self.name + '/gas', Float64, queue_size=1)
        self.wind_pub = rospy.Publisher('/' + self.name + '/wind', Float32MultiArray,
                                        queue_size=1)  # publish wind speed and direction
        self.scan_pub = rospy.Publisher('/' + self.name + '/scan', scan_msg, queue_size=1)
    ...
    def publish_gas(self):
        self.gas_pub.publish(self.sensed_odor)

    def publish_wind(self):
        msg = Float32MultiArray()
        self.wind_lock.acquire()
        msg.data = [self.upwind_dir,self.downwind_dir,self.crosswind_dir, self.global_upwind_dir, self.global_downwind_dir,self.wind_speed]
        self.wind_lock.release()
    ...
    def publish_scan(self):
        msg = scan_msg()
        self.scan_lock.acquire()
        msg.n_readings = self.n_readings
        msg.ranges = [self.readings[i] for i in range(self.n_readings)]
        msg.FOV =self.FOV
        msg.start_angle = self.start_ang
        msg.angle_increment = self.ang_inc
        msg.max_range = self.laser_range
